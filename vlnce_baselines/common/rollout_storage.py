...
class CMARolloutStorage(RolloutStorage):
    """A RolloutStorage container for actions consisting of pano, offset, and
    distance components.
    """
    ...
    def __init__(
        ...
        self.prev_actions = torch.zeros(num_steps + 1, num_envs, 1).long()
    ...
    def insert(
        ...
        self.prev_actions[self.step + 1].copy_(action)
    ...
    def recurrent_generator(
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                prev_actions_batch.append(
                    self.prev_actions[: self.step, ind]
                )
            ...
            prev_actions_batch = self._flatten_helper(
                T, N, prev_actions_batch
            )
