...
from vlnce_baselines.models.waypoint_predictors import WaypointPredictionNet, ContrastiveNet
...
@baseline_registry.register_policy
class WaypointPolicyCL(WaypointPolicy, Policy):
    def __init__(
        self,
        observation_space: Space,
        action_space: Space,
    ...
    def act(
        ...
        pano_stop_distribution, pano_stop_distribution_cl = pano_stop_distribution
        pano_stop = (
            pano_stop_distribution.mode()
            if deterministic
            else pano_stop_distribution.sample()
        ...
        action_log_probs = pano_stop_distribution_cl.log_prob(pano_stop)
    ...
    def evaluate_actions(
        ...
        pano_stop_distribution, pano_stop_distribution_cl = pano_stop_distribution
        value = self.critic(x)
        pano_log_probs = pano_stop_distribution_cl.log_prob(
            action_components["pano"]
        )
        ...
        entropy = {
            "pano": pano_stop_distribution_cl.entropy(),
            "offset": (o_mask * offset_distribution.entropy()).squeeze(1),
            "distance": (d_mask * distance_distribution.entropy()).squeeze(1),
        }
